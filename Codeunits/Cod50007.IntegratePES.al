codeunit 50007 "PES Integration"
{
    trigger OnRun()
    var
    begin
    ...
    begin
        ...
        LogId := Utility.InsertLog(IntegrationType::"MTwo To BC", IntegrationFn::"Import PES", RequestData, LogText);
        ...
        Utility.ValidatePESData(RequestData, LogText, LogId);
        ...
        if ErrorExists then begin
            ...
            Utility.ModifyLogText(LogId, LogText);
            ClearLastError();
            Utility.CreateJournalForPES(RequestData);
            LogText.AppendLine(PESJouenalInserted);
            LogText.AppendLine(BindingJSONResponse);
            Utility.ModifyLogText(LogId, LogText);
            JSONResponse := Utility.BindSucessJSONResponse('PES Journal');
            LogText.AppendLine(InsertingLog);
            Utility.ModifyLogText(LogId, LogText);

codeunit 50011 "Integrate Customer Invoice"
{
    trigger OnRun()
    var
    begin
    ...
    begin
        ...
        LogId := Utility.InsertLog(IntegrationType::"MTwo To BC", Enum::"Integration Function"::"Import Customer Invoice", RequestData, LogText);
        ...
        Utility.ValidateCustomerInvoice(RequestData, LogText, LogId);
        ...
        if ErrorExists then begin
            ...
            Utility.ModifyLogText(LogId, LogText);
            ClearLastError();
            Utility.CreateJournalForCustomerInvoice(RequestData);
            LogText.AppendLine(InvoiceInserted);
            LogText.AppendLine(BindingJSONResponse);
            Utility.ModifyLogText(LogId, LogText);
            JSONResponse := Utility.BindSucessJSONResponse('Customer Invoice');
            LogText.AppendLine(InsertingLog);
            Utility.ModifyLogText(LogId, LogText);
    ...
    var
        ...
        ValidatingInvocie: Label '*********************** Validating Customer Invoice Fields ***********************';
        ErrorFound: Label '*********************** Found Error ***********************';
        BindingErrorResponse: Label '*********************** Binding JSON Error Response ***********************';
        ResponseSent: Label '*********************** JSON Response Sent ***********************';
        InsertingInvoice: Label '*********************** Inserting Customer Invoice in Gen. Journal line ***********************';
